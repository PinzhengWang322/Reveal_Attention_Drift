...
class BaseEditor:
    ...
    def __init__(self,
        ...
        if 'gpt' in self.model_name.lower():
            ...
            if self.model_name == "gptj-20B":
                model_path = "/path/to/gpt-neox-20b"
            self.model = AutoModelForCausalLM.from_pretrained(model_path, torch_dtype=torch.bfloat16, device_map='auto' if hparams.model_parallel else None)
            self.tok = GPT2Tokenizer.from_pretrained(model_path)
            self.tok.pad_token_id = self.tok.eos_token_id
        elif 'llama13b' in self.model_name.lower():
            model_path = "/path/to/Llama-2-13b-hf"
            ...
            model_path = "/path/to/TinyLlama-1.1B-pretrain-3T"
