...
def compute_v(
    ...
    def kl_divergence(p, q, input_lens):
    ...
    for it in range(hparams.v_num_grad_steps):
        ...
        with nethook.TraceDict(
            ...
            output = model(**input_tok, output_attentions=True)
        ...
        if ori_attn_W is None:
            ...
            if hparams.high_attn_range is None:
                attn_W_loss = sum([loss_function(p.detach(), q, predict_idxs) for q, p in zip(attn_W, ori_attn_W)]) * hparams.attn_W_loss_weight * 1e-4
            else:
                attn_W_loss = sum([loss_function(p.detach(), q, predict_idxs) for lidx, (q, p) in enumerate(zip(attn_W, ori_attn_W))
